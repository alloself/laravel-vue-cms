<template>
  <v-container fluid>
    <v-card>
      <v-card-title>{{ createView ? 'Создать': `# ${$route.params.id}` }} {{ createView ? 'блок карусели' :'' }}</v-card-title>
      <v-card-text>
        <v-form>
          <v-text-field
            outlined
...
export default {
  ...
  methods: {
    ...
    async create() {
      ...
      this.$router.push({ name: "CarouselBlockDetail", params: { id } });
    },
    async update() {
    ...
    async remove() {
      ...
      this.$router.push({ name: `carouselBlocks` });
    },
  },
  async updated() {
